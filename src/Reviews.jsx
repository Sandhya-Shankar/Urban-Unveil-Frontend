...
const Reviews = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`https://backend-8e8u.onrender.com/getReview/${restID}`)
      .then(response => {
  ...
  const handleSubmit = () => {
    ...
    axios.post(`https://backend-8e8u.onrender.com/createReview/${restID}`, {name, review: reviewText, rating })
      .then(response => {
    ...
    <div>
      ...
      <div style={{ marginTop: '20px', padding: '20px', backgroundColor: '#f8f9fa', borderRadius: '5px' }}>
        ...
        {reviews.length > 0 ? (
          <ul>
            {reviews.map(reviews => (
              <li key={reviews.restID}>
              <div style={{ marginBottom: '20px', padding: '10px', backgroundColor: '#ffffff', border: '1px solid #ced4da', borderRadius: '5px' }}>
                <h3 style={{ marginBottom: '10px' }}>{reviews.name}</h3>
                <p style={{ marginBottom: '5px' }}>Review: {reviews.review}</p>
                <p style={{ marginBottom: '0' }}>Rating: {reviews.rating}</p>
              </div>
            </li>
